...
// 테스트용: 샘플 수를 16으로 줄여서 DMA가 몇 워드를 덮어쓰는지 확인
#define RX_BUFFER_BASE      0x01000000U
#define NUM_SAMPLES         16U
#define BYTES_PER_SAMPLE    4U
#define DMA_TRANSFER_SIZE   (NUM_SAMPLES * BYTES_PER_SAMPLE)   // 16 * 4 = 64 bytes
...
/* ---------------- RX 버퍼를 패턴으로 채우기 (DMA 전 테스트용) ---------------- */
void fill_rx_buffer_with_pattern(u32 pattern)
{
    u32 *buf = (u32 *)RX_BUFFER_BASE;

    xil_printf("Filling RX buffer with pattern 0x%08lx ...\r\n", pattern);
...
void print_rx_data(void)
{
    ...
    xil_printf("First %d samples in DDR:\r\n", (int)NUM_SAMPLES);
    for (u32 i = 0; i < NUM_SAMPLES; i++) {
        xil_printf("[%2lu] 0x%08lx\r\n", i, buf[i]);
    }
}
...
void print_decoded_samples(void)
{
    ...
    xil_printf("Decode first %d samples (lower/upper 18 bits):\r\n", (int)NUM_SAMPLES);
    ...
    for (u32 i = 0; i < NUM_SAMPLES; i++) {
        ...
        xil_printf("[%2lu] raw=0x%08lx  low18=0x%05lx  high18=0x%05lx\r\n",
                   i, raw, low18, high18);
    }
}
...
void print_voltage_samples(void)
{
    ...
    xil_printf("First %d samples as code & voltage:\r\n", (int)NUM_SAMPLES);
    for (u32 i = 0; i < NUM_SAMPLES; i++) {
        u32 raw = buf[i];
        ...
        xil_printf("[%2lu] raw=0x%08lx  code=%6ld  Vdiff=%4ld mV\r\n",
                   i, raw, code, v_mv);
    }
}
...
int main(void)
{
    ...
    // ★ 테스트용: RX 버퍼 전체를 0xAAAAAAAA 패턴으로 채운 뒤, DMA가 몇 개를 덮어쓰는지 확인
    fill_rx_buffer_with_pattern(0xAAAAAAAAU);
    ...
    while (1) {
        // 이후 여기서 반복 DMA, 이더넷 송출 등으로 확장 가능
    }
